...
import { ChatMessage, AttendeeProfile } from '../../types'; // Sửa đường dẫn import
import { chatWithAria, summarizeAndSaveProfile } from '../../services/aiService'; // Dùng service mới

interface WhatsAppSimulationProps {
  userId: string; // Thêm prop userId để biết lưu cho ai
  onComplete: (profile: AttendeeProfile) => void;
  onDecline: () => void;
}

const WhatsAppSimulation: React.FC<WhatsAppSimulationProps> = ({ userId, onComplete, onDecline }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const responseText = await chatWithAria(newHistory);
      
      setIsTyping(false);
      
      // Kiểm tra xem AI đã trả về JSON (dấu hiệu hoàn tất) chưa
      ...
      if (jsonMatch) {
        ...
        // Gọi Backend để tóm tắt và lưu vào Supabase
        const profileData = await summarizeAndSaveProfile(newHistory, userId);
      ...
      console.error(error);
      setIsTyping(false);
      setMessages(prev => [...prev, { role: 'model', text: "I apologize, connection error. Please try again.", timestamp: Date.now() }]);
    }
  };
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {isDeclined && (
          <div className="bg-white/90 p-6 rounded-2xl border border-red-100 text-center space-y-4 animate-in zoom-in-95 duration-300">
            ...
            <button onClick={onDecline} className="w-full py-3 bg-slate-900 text-white rounded-xl font-bold text-sm">
